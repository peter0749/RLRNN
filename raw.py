...
if __name__ == "__main__":
    agent = PGAgent(lr=0.001, gamma=0.99)
    #agent.load(str(sys.argv[1]))
    #seedPos = str(sys.argv[2])
    rewardSys = rewardSystem() ## more sensitive
    done = False
    batch_size = 128

    with open('./raw.csv', 'a+', 0) as logFP: ## no-buffer logging
        ...
        for e in xrange(EPISODES):
            ...
            agent.train()
            if e % 10 == 0:
                agent.save("./raw/melody-ddqn-{}.h5".format(e))
